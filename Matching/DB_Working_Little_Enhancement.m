...
img_file = imread('F:\PhD\Software\Matlab\Simple_FingerPrint_Matching\t1.bmp');
img=~img_file;
%cd  'H:\PhD\Software\Matlab\Simple_FingerPrint_Matching\New Folder';
...
plot(CentroidBif(:,1),CentroidBif(:,2),'go');
hold off
[m,n]=size(CentroidTerm);
term_count=m; 
for i=1:term_count
    Term_out(i, 1) = CentroidTerm(i, 1);
    Term_out(i, 2) = CentroidTerm(i, 2);
end
[m,n]=size(CentroidBif);
bif_count=m; 
for i=3:bif_count-2
    Bif_out(i-2, 1) = CentroidBif(i, 1);
    Bif_out(i-2, 2) = CentroidBif(i, 2);
end
[m,n]=size(Term_out);
[x,y]=size(Bif_out);
for i=1:m
    out(i, 1) = Term_out(i, 1);
    out(i, 2) = Term_out(i, 2);
end
for i=m+1:m+x
    out(i, 1) = Bif_out(i-m, 1);
    out(i, 2) = Bif_out(i-m, 2);
end
[m,n]=size(out);
...
minu_count=m; 
 row=800;
 col=768;
 minutiae_i = 255 * ones(row,col,1,'uint8');
%imshow(minutiae_i);
  for i=1:minu_count 
    x1 = out(i, 1);
    y1 = out(i, 2);
    %minutiaeimg(minu_count, :) = [ x, y];
        for k = y1-1: y1 + 1
          for l = x1-1: x1 + 1
          minutiae_i(k, l,:)= [0]; 
          end
