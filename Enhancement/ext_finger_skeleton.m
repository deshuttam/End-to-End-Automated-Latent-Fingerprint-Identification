...
function [ ret ] = ext_finger_skeleton( img, display_flag, filename )
    res = num2str(filename);  
    fn=sprintf('%s',res);
...
% Enhancement -------------------------------------------------------------
    ...
    for x=1:55
       if numel(find(img(x,:)<200)) < 8   %if numel(find(img(x,:)<200)) < 8
  ...
  [ binim, mask, cimg, cimg2, orient_img, orient_img_m ] = f_enhance_3420_NIST(img);
  %[ binim, mask, cimg, cimg2, orient_img, orient_img_m ] = f_enhance_fvc2004(img);
   %[ binim, mask, cimg, cimg2, orient_img, orient_img_m ] = f_enhance_NISTAE(img,ind);
% Making Mask -------------------------------------------------------------
...
% Return Minutiae ---------------------------------------------------------
    if display_flag == 1
        ...
        imwrite(combined,fn); %save minutiae image
    end
   % imwrite(minutiae,fn);  %save minutiae image with orientations
    ret=minutiae;
end